...
pub fn draw(frame: &mut Frame, area: Rect, state: &mut AppState) {
    ...
    let hints = Paragraph::new(" [i] Add  [d] Delete  [Space] Status  [f] Filter  [u] Update  [Tab] Settings  [q] Quit")
...
fn draw_side_panel(frame: &mut Frame, area: Rect, state: &mut AppState) {
    ...
    let Some((title, status, progress, tags, description)) = state.library.selected_book().map(|book| (
        book.title.clone(),
        book.status.clone(),
        book.progress.clone(),
        book.tags.clone(),
    ...
    let desc = description.as_deref().unwrap_or("No description available.");
