...
pub fn handle_installing_plugin(
    ...
    {
        let url = if let Modal::InstallPlugin { url, .. } = &state.modal {
            url.trim().to_string()
        } else {
            String::new()
        };
...
mod tests {
    ...
    fn setup_install_plugin_state(url: &str, cursor: usize) -> AppState {
        let mut state = test_state();
        state.modal = Modal::InstallPlugin {
            url: url.to_string(),
            cursor,
            scroll_offset: 0,
        };
        state.current_page = Page::InstallingPlugin;
        state
    }
    ...
    fn test_handle_installing_plugin_enter_submits() {
        ...
        let (tx, rx) = channel();
        let result = handle_installing_plugin(&mut state, key_event(KEY_ENTER), &tx);
        assert!(result);
    ...
    fn test_handle_installing_plugin_types_characters() {
        ...
        let (tx, _rx) = channel();
        handle_installing_plugin(&mut state, key_event(KeyCode::Char('g')), &tx);
        handle_installing_plugin(&mut state, key_event(KeyCode::Char('h')), &tx);
        if let Modal::InstallPlugin { url, cursor, .. } = &state.modal {
    ...
    fn test_handle_installing_plugin_backspace() {
        ...
        let (tx, _rx) = channel();
        handle_installing_plugin(&mut state, key_event(KEY_BACKSPACE), &tx);
        if let Modal::InstallPlugin { url, cursor, .. } = &state.modal {
            assert_eq!(url, "https://");
            assert_eq!(*cursor, 8);
        } else {
    ...
    fn test_handle_installing_plugin_left_right_navigation() {
        ...
        let (tx, _rx) = channel();
        handle_installing_plugin(&mut state, key_event(KeyCode::Left), &tx);
        if let Modal::InstallPlugin { cursor, .. } = &state.modal {
            assert_eq!(*cursor, 0);
        } else {
            panic!("Expected InstallPlugin modal");
        }

        handle_installing_plugin(&mut state, key_event(KeyCode::Right), &tx);
        if let Modal::InstallPlugin { cursor, .. } = &state.modal {
            assert_eq!(*cursor, 1);
        } else {
            panic!("Expected InstallPlugin modal");
    ...
    fn test_handle_installing_plugin_home_end() {
        ...
        let (tx, _rx) = channel();
        handle_installing_plugin(&mut state, key_event(KeyCode::Home), &tx);
        if let Modal::InstallPlugin { cursor, .. } = &state.modal {
            assert_eq!(*cursor, 0);
        } else {
            panic!("Expected InstallPlugin modal");
        ...
        handle_installing_plugin(&mut state, key_event(KeyCode::End), &tx);
        if let Modal::InstallPlugin { cursor, .. } = &state.modal {
            assert_eq!(*cursor, 6);
        } else {
            panic!("Expected InstallPlugin modal");
    ...
    fn test_handle_installing_plugin_delete() {
        ...
        let (tx, _rx) = channel();
        handle_installing_plugin(&mut state, key_event(KeyCode::Delete), &tx);
        if let Modal::InstallPlugin { url, cursor, .. } = &state.modal {
            assert_eq!(url, "abd");
            assert_eq!(*cursor, 2);
        } else {
